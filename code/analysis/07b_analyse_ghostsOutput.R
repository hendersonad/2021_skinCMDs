...
make_regression_tab <- function(exposure){
  ...
  # load data ---------------------------------------------------------------
  df_model_anx <-
    readRDS(paste0(datapath, "out/models_data/df_model", ABBRVexp, "_anxiety_noghosts.rds"))
  
  df_model_dep <-
    readRDS(paste0(datapath, "out/models_data/df_model", ABBRVexp, "_depression_noghosts.rds"))
...
tab3_out %>%
  gt::gtsave(
    filename =  paste0("table4_regression_noghosts.html"),
    path = here::here("out/analysis")
  )
tab3_out %>%
  gt::gtsave(
    filename =  paste0("table4_regression_noghosts.rtf"),
    path = here::here("out/analysis")
  )
