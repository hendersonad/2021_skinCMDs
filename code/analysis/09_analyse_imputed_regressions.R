...
for (exposure in XX) {
  ...
  if (exposure == "eczema") {
    ...
    var_consbeforeindex <-
      haven::read_dta(paste0(datapath, "out/variables-ecz-consultations-yrbeforeindex.dta"))
    ...
    var_consbeforeindex <-
      haven::read_dta(paste0(datapath, "out/variables-pso-consultations-yrbeforeindex.dta"))
  }
  ...
  for (outcome in YY) {
    ...
    saveRDS(df_validsets_noghosts, file = paste0(datapath, "out/models_data/df_model", ABBRVexp, "_", outcome, "_noghosts.rds"))
