...
#     prop.test(n = length(df_exp_tuc$out), x = sum(df_exp_tuc$out))
#     
...
#     
#     covars <- c("age", "gender", "years", 
#                 "carstairs", "cal_period", "cci",
#                 "bmi2", "smoker", "alc")
#     
#     univ_roc <- function(covariate){
#       uni_1 <- glm(out ~ get(covariate), data = df_exp_train, family = "binomial")
#       pred_vals <- predict(uni_1, type = "response")
#       
#       roc_data <- df_exp_train %>% 
#         select(setid, patid, out, all_of(covariate)) %>% 
#         filter(!is.na(get(covariate))) %>% 
#         mutate(predicted = pred_vals)
#       
#       # report AUC  -------------------------------------------------------------
#       roc_calc <- pROC::roc(roc_data$out, roc_data$predicted)
#       roc_calc$auc
#       plot(roc_calc, main = covariate)
...
#     multi_1 <- glm(out ~ age + gender + carstairs + cal_period + cci + bmi2 + smoker + alc, data = df_exp_train, family = "binomial")
#     #broom::tidy(multi_1, exp= T)
#     
#     pred_vals <- predict(multi_1, type = "response")
#     roc_data <- multi_1$model %>% 
#       mutate(predicted = pred_vals)
#     
#     # report AUC (in train and test) ------------------------------------------
#     par(mfrow = c(1,2))
#     roc_calc <- pROC::roc(roc_data$out, roc_data$predicted)
#     roc_calc$auc
#     plot(roc_calc, main = "Train")
#       text(1, 1, round(roc_calc$auc,4), font = 2, pos = 4)
#       
#     ##
#     pred_vals_test <- predict(multi_1, type = "response", newdata = df_exp_test)
#     roc_data_test <- df_exp_test %>% 
#       mutate(predicted = pred_vals_test)
#     
#     roc_calc_test <- pROC::roc(roc_data_test$out, roc_data_test$predicted)
#     roc_calc_test$auc
#     plot(roc_calc_test, main = "Test")
#     text(1, 1, round(roc_calc_test$auc,4), font = 2, pos = 4)
#     dev.copy(pdf, paste0(here("out/predictions/"), "02_multi_auc_", ABBRVexp, "_", substr(outcome, 1, 3), ".pdf"), 8, 8)
#       dev.off()
#   }
# }
# 
# rm(roc_calc_test, roc_calc, roc_data, roc_data_test, multi_1, uni_1, df_exp_test, df_exp_train)

# Redo with Cox and survival models to get curves ----------------------
for(exposure in XX) {
  ABBRVexp <- substr(exposure, 1, 3)
  for(outcome in YY) {
    ...
    plot_roc <- function(test_train) {
      for(tt in seq(365, 365*2, 365*5)){
        ...
        if(tt==365){
          ii <- 1
          plot(roc_calc, main = str_to_title(test_train))
          text(1, 1, round(roc_calc$auc,2), font = 2, pos = 4)
          legend("bottomright", legend = c("1 year", "3 year", "5 year"), col = 1:3, lty = 1, bty = "n")
        }else{
          lines(roc_calc$specificities, roc_calc$sensitivities, col = ii+1)
    ...
    dev.copy(pdf, paste0(here("out/predictions/"), "04_Cox_multi_auc_", ABBRVexp, "_", substr(outcome, 1, 3), ".pdf"), 8, 8)
      dev.off()
  }
}

# predict 1-year, 3-year and 5-year risk  ---------------------------------
...
# remove time-updating vars and make constant ---------------------------------
## identify subgroups whose risk increases by >50% 
  ...
  for(outcome in YY) {
    ...
    if(length(ordered_vars) > 0){
      for(varname in ordered_vars) {
        print(varname)
        x_ordered <- df_exp_surv[[varname]]
        x_fact <- factor(x_ordered, ordered = FALSE)
        df_exp_surv[[varname]] <- x_fact
      }
    }
    ...
    ## plot_boxplot of risk scores 
      ...
      text(0.75, 0.5, text2, pos = 4, cex = 0.7)
  }
}
    dev.off()
