...
cohort_sleep <- cohort_severity %>% 
  left_join(severity, by = "patid") %>% 
  dplyr::select(setid, patid, exposed, indexdate, severity) %>% 
...
p3 <- tab_sleep_out %>% 
	  ...
	  mutate(sleep_class = 
	           factor(sleep_class, 
	                  levels = c("read", "drugs_poss", "drugs_def"), 
	                  labels = c("All sleep Read codes", "All sleep drugs", "Definite sleep drugs"))) %>% 
