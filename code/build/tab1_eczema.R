...
var_depressionDef <- haven::read_dta(paste0(datapath, "out/variables-ecz-depression-definite.dta"))
var_anxietyDef <- haven::read_dta(paste0(datapath, "out/variables-ecz-anxiety-definite.dta"))
var_depressionAll <- haven::read_dta(paste0(datapath, "out/variables-ecz-depression-all.dta"))
var_anxietyAll <- haven::read_dta(paste0(datapath, "out/variables-ecz-anxiety-all.dta"))
var_depressionCensor <- haven::read_dta(paste0(datapath, "out/variables-ecz-depression-censor.dta"))
var_anxietyCensor <- haven::read_dta(paste0(datapath, "out/variables-ecz-anxiety-censor.dta"))
var_agesexgp <- haven::read_dta(paste0(datapath, "out/variables-ecz-age-sex-gp", ".dta"))
var_bmi <- haven::read_dta(paste0(datapath, "out/variables-ecz-BMI-all.dta"))
var_eth <- haven::read_dta(paste0(datapath, "out/variables-ecz-ethnicity.dta"))
var_alc <- haven::read_dta(paste0(datapath, "out/variables-ecz-harmfulalcohol.dta"))
var_smok <- haven::read_dta(paste0(datapath, "out/variables-ecz-smoke-all.dta"))
var_sev <- haven::read_dta(paste0(datapath, "out/variables-ecz-severity.dta"))
...
glimpse(df_tab1)
df_tab1 %>%
	filter(!is.na(alcdate))
write_parquet(df_tab1, sink = paste0(datapath, "out/df_tab1_eczema.gz.parquet"))
#df_tab1 <- read_parquet(file = paste0(datapath, "out/df_tab1_eczema.gz.parquet"))
...
df_tab1 <- df_tab1 %>%
	...
	mutate_at(c("exposed", "gender", "dep", "anx", "eth5", "eth16", "alc", "smokstatus", "modsevere"), ~as_factor(.))
...
tab1 <- df_tab1 %>% 
	select(patid, exposed, futime, gender, age, age_cat, eth5, bmi, bmi_cat,
				 dep, anx, alc, smokstatus, modsevere) %>%
...
tab1 <- tab1 %>% 
	mutate(gender = factor(gender, levels = c("Male", "Female"), exclude = c("Data Not Entered", "Indeterminate", "Unknown")),
				 ...
				 severe = factor(modsevere, levels = levels(df_tab1$modsevere), labels = c("Mild", "Moderate", "Severe")))
...
table1 %>%
	as_flex_table() %>%
	flextable::save_as_docx(path = here::here("out/tab1_eczema.docx"))
